...
for my $min_avr(1..15) {
    for my $min_samples(1..15) {
        $cmd = "./generate_params.pl $last_week $test_week_num $min_avr $min_samples";
        print STDERR "$cmd\n";
        system $cmd;
      ...
      R_SQUARED_LOOP:
        for my $r_squared_start_index(0..$#r_squared_values - 1) {
            ...
            for (my $bet = 0.1; $bet < 0.2; $bet += 0.1) {
                ...
                print "Sum: $sum\n";
                print "Final capital: $capital k_value: $k_value min_avr: $min_avr min_samples $min_samples bet: $bet r_squared: $r_temp[0]\n";
                print qq{======================================================================\n};
            }
            last;
        }
