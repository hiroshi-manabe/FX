...
foreach my $window_time (60000, 120000, 180000, 240000, 300000) {
    foreach my $r_squared_value (map { 0.92 + $_ * 0.0025 } 0 .. 20) {
        my $training_start_week = 0;
        while ($training_start_week <= 21) {
            ...
            foreach my $k_value (5 .. 10) {
                foreach my $threshold_value (1 .. $k_value - 1) {
                    ...
                    my $cmd = "python3 $test_script $training_start_week " . ($training_start_week + 28) . " " . ($training_start_week + 29) . " " . ($training_start_week + 29) . " --k_value $k_value --threshold_value $threshold_value --window_time $window_time --r_squared_value $r_squared_value> $output_file";
                    print "$cmd\n";
                    system($cmd);
                }
            }
