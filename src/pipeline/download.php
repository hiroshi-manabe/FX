...
foreach ($download as $symbol)
{
    ...
    try {
        ...
        if ($weeksToGoBack > 0) {
            ...
            $startDate->modify("-{$weeksToGoBack} weeks");  // Set if weeksToGoBack > 0
        }
    } catch (Exception $e) {
    ...
    if ($startDate === null) {
        logger('Start date is not set, downloading all available data.');
    } else {
        logger('Start date is set to: ' . $startDate->format('Y-m-d H:i:s') . ' UTC');
    }

    while ($startDate === null || $currentDateTime >= $startDate)
    {
