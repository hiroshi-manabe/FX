...
sub main {
    ...
    for my $currency(@currency_list) {
        ...
        while (<$currency/weekly_past_sell_data/week_*.csv>) {
            my %processed_dict = ();
            m{week_(\d{3})};
            ...
            for my $i(0..$#data) {
                if ($i >= $delay) {
                    ...
                    my $past = $data[$i-$delay]->[2];
                    next if $past eq "";
                    my ($result, $result_time) = split/:/, $data[$i]->[1];
                    next if $result eq "";
        ...
        open OUT, ">", "$currency/stat_sell.csv";
        for my $i(0..255) {
            my $hex = sprintf("%02x", $i);
            open $fp_dict{$hex}->{"fp"}, "<", $fp_dict{$hex}->{"file"} or die;
            my %dict = ();
