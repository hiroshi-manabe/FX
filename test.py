...
def process_matching_points(train_data, dev_data):
    profit_sum = 0
    trade_count = 0
    for row in dev_data:
        first_coef, second_coef, buy_result, sell_result = row
        
        knn_buy = k_nearest_neighbors("buy", first_coef, second_coef, train_data, k=k_value, threshold=threshold_value)
        knn_sell = k_nearest_neighbors("sell", first_coef, second_coef, train_data, k=k_value, threshold=threshold_value)
        ...
        if action != "パス":
            profit = data[index][2 if action == "buy" else 3]
            print(f"利益: {profit}")
            profit_sum += profit
            trade_count += 1
...
def main(start_train_week, end_train_week, start_dev_week, end_dev_week, k_value=8, threshold_value=5):
    ...
    process_matching_points(train_data, dev_data)
