...
sub main() {
    die "command <bit_width> <time_width> <bit_height> <pips_height> <overwrite>" if @ARGV != 5;
    my ($bit_width, $time_width, $bit_height, $pips_height, $overwrite) = @ARGV;
    for my $currency(@currency_list) {
        while (<$currency/weekly_sell_data/week_*.csv>) {
            ...
            my $cmd = qq{./add_past_sell_data $bit_width $time_width $bit_height $pips_height < $_ > $file_to_write_temp};
            print "$cmd\n";
            system $cmd;
